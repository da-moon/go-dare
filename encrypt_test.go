...
func TestDare(t *testing.T) {
	...
	t.Run("decrypt", func(t *testing.T) {
		buf := bytes.NewBuffer(expectedEncryptionResult)
		reader := bufio.NewReader(buf)
		enc := decryptor.NewReader(
			reader,
			nonce,
			sharedKey,
		)
		...
		assert.Equal(t, len(testString), int(n))
		assert.True(t, bytes.Equal(testString, result.Bytes()), fmt.Sprintf("want: %v \n got: %v", testString, result.Bytes()))
